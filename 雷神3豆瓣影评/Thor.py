...
firstUrl = 'https://movie.douban.com/subject/25821634/comments?start=23978&limit=20&sort=new_score&status=P&percent_type='
url_beg = 'https://movie.douban.com/subject/25821634/comments'


def store(name, isread, title, star, vote, time, comment):
    csvFile = open("Thor.csv", 'a+', newline = '')
    try:
...
def getNextUrl(bsObj):
    nextUrl = bsObj.find("div", {"id": "paginator"})
    if nextUrl.find("a", {"class": "next"}) is not None:
        nextUrl = url_beg + nextUrl.find("a", {"class": "next"}).attrs['href']
    else:
        nextUrl = None
    return nextUrl
...
def start(onePage):
    # print(onePage)
    storeUrl(onePage)
    req = requests.get(url = onePage, cookies = cookies, headers = head)
    html = req.text
    bsObj = BeautifulSoup(html, 'lxml')
    getData(bsObj)
    nxtUrl = getNextUrl(bsObj)
    if nxtUrl is not None:
        start(nxtUrl)
